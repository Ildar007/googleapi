...
const config ={ 
    apiKey: process.env.VITE_APP_GMAP_API_KEY};
const mapApiJs = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyAR8phEVD88GQDnNy8xYBX6PpZTz2r67aI&libraries=places';
...
function AppiSite() {
  ...
  const initMapScript = () => {
    ...
    const src = `${mapApiJs}?key=${config}&libraries=places&v=weekly`;
  ...
  const reverseGeocode = ({ latitude: lat, longitude: lng}) => {
    const url = `${geocodeJson}?key=${config}&latlng=${lat},${lng}`;
